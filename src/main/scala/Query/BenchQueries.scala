...
object BenchQueries extends Enumeration {
  ...
  // 6 table join
  val QueryThree =
    """SELECT N_NAME, SUM((L_EXTENDEDPRICE*(1-L_DISCOUNT))) AS SUM_REVENUE
  ...
  // 2 table
  val QueryFive =
    """SELECT SUM(L_EXTENDEDPRICE) AS SUM_YEARLY FROM LINEITEM
  ...
  // 3 table
  val QuerySix =
    """SELECT SUM((L_EXTENDEDPRICE*(1-L_DISCOUNT))) AS SUM_REVENUE, O_SHIPPRIORITY, O_ORDERSTATUS
      |FROM CUSTOMER JOIN ORDER ON CUSTOMER.C_CUSTKEY = ORDER.O_CUSTKEY
      |JOIN LINEITEM ON LINEITEM.L_ORDERKEY = ORDER.O_ORDERKEY
      |WHERE O_ORDERDATE < '1995-03-15'
      |GROUP BY O_SHIPPRIORITY, O_ORDERSTATUS""".stripMargin.toLowerCase()
}
